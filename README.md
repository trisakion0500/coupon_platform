...
- 🚧 Frontend 구현 진행 중(`docs/19_DEV_SETUP.md`, React 18 + TypeScript + Vite + Ant Design + Zustand + Axios)
  ...
  - ✅ 관리메뉴 — 회사(SCR-010~012, 목록/등록/상세수정), 프로젝트(SCR-020~022, 목록/등록/상세수정 + API Secret 재발급 — `edit_count` 낙관적 락, 등록·재발급 시 평문 Secret 1회 노출 모달) 실제 백엔드 연동 + SUPER_ADMIN·DEVELOPER 권한별 화면(조회전용/스코핑) 브라우저 라이브 검증 완료
  - ⬜ 사용자·감사로그 관리메뉴, 캠페인 목록·등록·상세, 쿠폰 사용 로그 — 라우팅만 연결된 stub 상태, 다음 단계에서 도메인별로 구현 예정

### 향후 개선사항 (우선순위 낮음, 별도 검토 필요)

- **Swagger 문서화 제대로 붙이기** — `@nestjs/swagger`/`swagger-ui-express`는 이미 설치돼 있고 `SWAGGER_ENABLED=true`일 때 `/docs`로 노출되지만, DTO에 `@ApiProperty()` 등 데코레이터가 전혀 없고 `nest-cli.json`에 swagger CLI 플러그인도 등록 안 돼 있어 현재는 엔드포인트 목록 정도만 뜨고 요청/응답 필드 스키마는 거의 비어있다. 전체 DTO에 `@ApiProperty()`(설명/예시 포함)를 붙여 실제로 쓸 수 있는 API 문서 수준으로 만들 것.
