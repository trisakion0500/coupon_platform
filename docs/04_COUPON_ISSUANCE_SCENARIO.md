...
본 문서는 관리자가 쿠폰 캠페인을 만들고 실제 쿠폰 코드를 발급하는 흐름(캠페인 생성 → 코드 발급 → 생성 실패 시 재시도)을 정리한다. API 엔드포인트/result 코드 등 상세 스펙이 아니라 **흐름 자체의 설계 근거**를 다룬다 — 상세 API 스펙은 별도 문서(TODO)로 정리한다.
...
POST /campaigns/{id}/codes   코드 발급(RANDOM 대량생성 또는 FIXED 목록 등록)
```
...
FIXED  : 관리자가 코드 목록을 직접 입력/업로드 → 동기 처리 (즉시 완료)
```
...
sequenceDiagram
    ...
    S-->>A: 201 Created (coupon_campaign_id)
    A->>S: POST /campaigns/{id}/codes (RANDOM 수량 또는 FIXED 코드목록)
...
flowchart TD
    ...
    T -- FIXED --> F1["요청받은 코드 목록 각각 INSERT 시도"]
    F1 --> F2{"코드별 성공/실패
    (중복 등)"}
    F2 --> F3["generation_status=완료, 코드별 결과를 응답에 포함"]
```
...
**아직 미확정(TODO)**:
- `POST /campaigns`, `POST /campaigns/{id}/codes`, `POST /campaigns/{id}/codes/retry`의 상세 요청/응답 스키마, result 코드
- FIXED 코드 등록 시 개별 코드 검증 규칙(중복 처리 방식 등)
- 캠페인 승인/반려 API 자체의 상세 스펙
