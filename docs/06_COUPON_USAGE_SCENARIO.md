...
| 사용자당 한도(`use_limit_per_user`) | 같은 유저가 서로 다른 코드로 동시 reserve 시 `COUNT(*)` 체크를 둘 다 통과해 한도 초과 가능 | `coupon_code_usage`에 먼저 INSERT한 뒤 `SELECT COUNT(*) FROM coupon_code_usage WHERE coupon_campaign_id=? AND game_user_id=? FOR UPDATE`(방금 넣은 행 포함)로 재확인해 초과 시 ROLLBACK — INSERT를 먼저 하는 순서가 중요하다(2026-07-22 수정, 아래 참고) |
| confirm 중복 호출 | 게임서버가 confirm을 재시도로 두 번 보낼 수 있음 | 별도 락 불필요 — `confirmed_at`을 두 번 써도 같은 결과(멱등)라 무해함 |
| reserve 재시도(멱등 체크, 1.2 참고) | 완전히 같은 요청(같은 코드+같은 유저)이 동시에 두 번 도착하면 둘 다 "기존 행 없음"을 보고 통과할 수 있음 | RANDOM은 뒤이은 코드 조건부 UPDATE가 코드 행 자체를 잠그므로 둘 중 하나만 통과한다(즉시 33001로 깔끔하게 갈림). FIXED는 코드 행에 이런 락이 없어 두 트랜잭션이 함께 한도 확인 단계까지 갈 수 있는데, 원래(2026-07-21까지) 이 단계를 "한도확인(SELECT...FOR UPDATE) → INSERT" 순서로 구현했더니 진짜 동시 요청 시 두 트랜잭션이 아직 행이 없는 같은 갭에 대해 서로 호환되는 갭락을 동시에 얻은 뒤 둘 다 INSERT를 시도하면서 insert-intention lock끼리 충돌해 데드락(1213)이 나 한쪽이 33003이 아니라 원시 50001(DATABASE_ERROR)을 받는 경로가 있었다(2026-07-22 전수감사에서 발견) — "결국 하나만 통과시켜 데이터는 항상 안전하다"는 정합성 자체는 깨지지 않았지만(InnoDB가 데드락을 감지해 반드시 한쪽을 롤백시키므로 한도 초과는 불가능), 진 쪽이 받는 응답이 의도한 33003이 아니었다. **해결(2026-07-22)**: 순서를 "INSERT → 한도확인(FOR UPDATE)"으로 뒤집었다 — 두 INSERT 자체는 서로 락 경합 없이 독립적으로 들어가고, 그 다음 FOR UPDATE 재확인이 상대방의 미커밋 행과 마주쳤을 때만 대기하므로(순수 대기, 순환 대기 아님) 데드락 확률이 크게 줄어든다. 그래도 극히 좁은 타이밍에 두 INSERT가 완전히 동시에 끝나면 이론상 데드락이 여전히 가능하지만(양쪽 FOR UPDATE가 서로의 미커밋 행을 동시에 필요로 하는 경우), 그 경우에도 진 쪽이 재시도하면 1.2의 멱등 체크가 이긴 쪽의 커밋된 행을 찾아 동일한 성공 응답을 재현하므로 결과적으로 수렴한다. 정리하면 데이터 정합성은 처음부터 깨진 적 없고(SP_COUPON_RESERVE.sql), 이번 수정은 "진 쪽이 어떤 응답을 받는가"의 품질을 개선한 것이다. |
