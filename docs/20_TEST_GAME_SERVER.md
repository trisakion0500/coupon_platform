...
로그(`app.log`)만 남기고 건너뛴다 — 6.2(reserve/confirm 재시도 개별 판정), 6.5(개별 에러 케이스
판정), 6.3(버스트 중 단 1건이라도 40001이면 그 버스트 전체의 성공건수/DB 사후검증을 스킵 —
일부만 외부 스로틀링에 걸리면 나머지 응답 분포가 순수 동시성 결과를 더 이상 반영하지 않으므로)
셋 다 적용된다.
