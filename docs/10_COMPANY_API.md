...
- 회사 관리메뉴(본 문서 2장) + 프로젝트 관리메뉴([11_PROJECT_API.md](./11_PROJECT_API.md) 2장) 전체 접근(생성/수정/Secret 발급 포함)
- 쿠폰 도메인 컨트롤 전체 가능([17_CAMPAIGN_API.md](./17_CAMPAIGN_API.md) 1.2 참고)
...
- 회사/프로젝트 관리메뉴(본 문서 2장, [11_PROJECT_API.md](./11_PROJECT_API.md) 2장) 접근 불가(20001) — 단, 헤더 선택용 API(본 문서 3장, [11_PROJECT_API.md](./11_PROJECT_API.md) 3장)는 예외적으로 접근 가능
- 쿠폰 도메인 컨트롤 즉시 가능(승인 불요), `user_role` 배정 프로젝트 단위 스코핑([17_CAMPAIGN_API.md](./17_CAMPAIGN_API.md) 1.2 참고)

### OPERATOR

- 회사/프로젝트 관리메뉴(본 문서 2장, [11_PROJECT_API.md](./11_PROJECT_API.md) 2장) 접근 불가(20001) — 단, 헤더 선택용 API(본 문서 3장, [11_PROJECT_API.md](./11_PROJECT_API.md) 3장)는 예외적으로 접근 가능
- 쿠폰 도메인 컨트롤 등록 시 승인요청 상태로 전환 — SUPER_ADMIN/DEVELOPER/MANAGER가 승인 가능, `user_role` 배정 프로젝트 단위 스코핑([17_CAMPAIGN_API.md](./17_CAMPAIGN_API.md) 1.2 참고)
...
회원가입 화면 전용. 로그인 전이라 `GET /companies`를 호출할 수 없어 신설된 공개 엔드포인트([09_AUTH_API.md](./09_AUTH_API.md) 4장 회원가입에서 참조). 활성(status=1) 회사만 조회하며, `/companies/{company_id}`보다 먼저 등록해야 하는 정적 경로다.
...
관리 콘솔 헤더의 회사/프로젝트 선택 콤보박스 전용 API. 2장의 관리메뉴 API와 달리 **모든 역할이 접근 가능**하다 — 회사/프로젝트 관리 권한이 없어도(MANAGER/OPERATOR), 현재 어느 회사·프로젝트 컨텍스트에서 작업 중인지는 알아야 쿠폰 도메인 화면이 동작하기 때문이다. 로그인 시 1회 로드하고, 이후 등록/수정 시점에 프런트엔드에서 직접 동기화한다. 프로젝트 단위 헤더 API(선택된 프로젝트의 실제 role_code 조회)는 [11_PROJECT_API.md](./11_PROJECT_API.md) 3장 참고.
