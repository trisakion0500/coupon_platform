...
| 메뉴               | SUPER_ADMIN | DEVELOPER      | MANAGER | OPERATOR |
| ------------------ | :---------: | :------------: | :-----: | :------: |
| 프로젝트 목록 조회 | O (전체)    | O (역할보유 프로젝트만) | -       | -        |
| 프로젝트 상세 조회 | O (전체)    | O (역할보유 프로젝트만) | -       | -        |
| 프로젝트 등록      | O           | -              | -       | -        |
| 프로젝트 수정      | O           | -              | -       | -        |
...
| 메뉴                     | SUPER_ADMIN | DEVELOPER                                                   | MANAGER | OPERATOR |
| ------------------------ | :---------: | :-----------------------------------------------------------: | :-----: | :------: |
| 레이트리밋 로그 목록 조회 | O (전체)    | O (자사만, 역할보유(role_code<=20) 배정 프로젝트로 추가 제한) | -       | -        |

감사 로그와 동일한 규칙([15_LOG_AUDIT_API.md](./15_LOG_AUDIT_API.md) 3장의 `project`/`user_role` 테이블 로그 스코핑과 동일)이다 — 상세 조회 화면은 없다(목록만, [09_AUTH_SECURITY.md](./09_AUTH_SECURITY.md) 2.8.4 참고). MANAGER/OPERATOR는 애초에 회사/프로젝트/사용자 관리메뉴 자체에 접근 권한이 없으므로 이 로그도 조회 대상이 아니다.
