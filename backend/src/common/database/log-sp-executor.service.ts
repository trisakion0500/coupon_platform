 ...
 * 분리된 별도 커넥션 풀을 쓴다(04_DEV_CONVENTIONS.md 1장) — 로그 적재가 메인 트랜잭션과 같은
...
export class LogSpExecutorService implements OnModuleDestroy {
  ...
  /**
   * 로그 SP를 호출하고 RESULT SELECT 규약을 파싱해 반환한다. 로그 **조회**(예: 15_LOG_AUDIT_API.md의
  ...
  /**
   * 로그 적재 SP를 호출하되 실패해도 절대 throw하지 않는다(04_DEV_CONVENTIONS.md 1장: 로그 적재
